...
const fetchSummary = async (emailThread, setSummary, setLoading) => {
    ...
    try {
        ...
        setSummary(data.summary); // Assuming the API returns an object with a summary property
    } catch (error) {
        console.error('Failed to fetch summary:', error);
        setSummary('Failed to load summary.');
    } finally {
        setLoading(false); // Disable loading state regardless of the outcome
    }
};
...
function Summary() {
    ...
    return (
        <section className='Summary'>
            <h1>Summary</h1>
            <main className='m-4'>{summary}</main>
            <Button 
                onClick={() => fetchSummary(emailThread,setSummary, setLoading)}
                disabled={loading}
                className='bttn'>
